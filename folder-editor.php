...
$query = "SELECT * FROM `folders` WHERE `uniqueId` = '$folderId' AND `owner` = '$username';";
$result = mysqli_fetch_assoc(mysqli_query($db, $query));
if($result){
    $folderName = $result['name'];
}else{
    header('Location:index.php?msg=Folder+Not+Found');
}
$msg ='';
if (isset($_GET['done'])) {
    echo '
...
echo '
<title>'.$folderName.' - '.$_SESSION['username'].' - Kabeers Drive</title>
  ...
  <header class="bmd-layout-header">
    <div id="navbar-search-flood" class="main_search_nav_wrapper navbar-search-flood navbar navbar-light bg-faded" style="background-color:#D6D7D9;box-shadow:none">
      ...
      <ul class="nav navbar-nav mr-auto ml-2 text-left" style="width:35%">
        <li data-toggle="drawer" data-target="#dw-p1" aria-expanded="false" class="navbar-brand text-dark ">'.$folderName.'</li>

      </ul>      
      <button onclick="window.history.back()" class="mdc-icon-button text-dark material-icons mdc-top-app-bar__action-item--unbounded" aria-label="Download">arrow_back</button>
...
<?php echo $msg;?>
                <div class="input-group" style="width:100%">
                    <div style="width:100%" class="input-group-prepend">
                        <form action="server/folder-text-editor.php" method="post" style="width:100%">
                            <input type="text" hidden value="<?php echo $_GET['id']; ?>" name="folderId">
